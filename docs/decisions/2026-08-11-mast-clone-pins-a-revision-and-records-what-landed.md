...
- ~~**`fleet-drift-report.py` does not read `repos`.**~~ Done in this change: the report gains an `Upstream repos` matrix, a fleet-summary verdict, a `RESULT:` line, and CSV columns. It distinguishes three states — `DIFFERS` (units resolved to different commits), `UNPINNED` (a rev was requested but the checkout is on a branch: an override, or a clone predating the pin), and `absent` (a role that never pulls that repo, which is not drift). What it still does not do is compare against the manifest's *expected* rev — "did the pin take" is a different question from "is the fleet consistent", and arguably belongs in a post-run check rather than a fleet report.
