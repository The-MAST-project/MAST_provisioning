...
- **The `.sh` half is now exercised too** (bash 5.3.15 installed for it), against the same throwaway-origin scenario, with identical results: clone leaves the pinned repo detached at the tag and the floating one on `master`; `--update` moves only the floating one; a `--branch` override displaces the pin, warns, and is recorded as unpinned in the sidecar; and the hand-written JSON parses with the same keys and shape as the PowerShell writer's. What remains untested is the *platform* rather than the logic — this ran on darwin, not on the Linux control/gui machines.
...
- ~~**`fleet-drift-report.py` does not read `repos`.**~~ Done in this change: the report gains an `Upstream repos` matrix, a fleet-summary verdict, a `RESULT:` line, and CSV columns. It distinguishes four states: `DIFFERS` (units resolved to different commits), `UNPINNED` (a rev was requested but the checkout is on a branch — an override, or a clone predating the pin), `MISSING` (expected for this unit's role per the manifest's `roles` column, and absent — a real gap, counted as drift), and `n/a` (not expected for this role, benign). The MISSING/`n/a` split was added on review: treating all absence as benign hid a genuine gap, since `claude` has roles `unit,control,spec` and a unit without it *is* missing something, while control-only `gui` is not. What it still does not do is compare against the manifest's *expected* rev — "did the pin take" is a different question from "is the fleet consistent", and arguably belongs in a post-run check rather than a fleet report.
