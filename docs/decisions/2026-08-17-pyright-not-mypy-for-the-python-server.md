...
- **The `types` job has never run on a runner.** Validated locally (0 errors, 0 warnings; the planted-error check in both directions) and the workflow YAML parses with four jobs, but as with the CI record, until it has executed in Actions this is YAML.
