...
- **Whether this becomes the fleet pattern** (`#73`'s last step) -- and the answer is mostly no. `MAST_common`, `MAST_gui` and `MAST_control` are Linux/cross-platform, where any PowerShell is incidental and not load-bearing, so they are not candidates (Eli, 2026-08-18). `MAST_unit` and `MAST_spec` may want it and probably do not. What remains genuinely open is only those two, and the exclusions here were argued from *this* repo's evidence -- the false-positive rates above are a property of a codebase built out of provider scriptblocks, and need re-measuring anywhere they are proposed.
