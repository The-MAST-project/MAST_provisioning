# MAST Provisioning - Architecture Decisions, 2026-05-04 to 2026-07-29 (archive)
...
> directory. The **91 entries below stay exactly as written**, in their original
> reverse-chronological order, and are not split into individual files.
>
> - **New decisions** go in a dated file here -- format: `README.md` in this directory.
> - **This file stays authoritative** for what it covers and must be **searched
>   alongside** the individual records. For everything it covers, it is the only place
>   that rationale exists.
> - **Why the move:** `2026-08-07-decision-records-are-one-file-each.md`.
>
> **The filename records this file's provenance, not its current span.** As `DECISIONS.md`
> it ran to 2026-08-03; the 27 entries written on `eli/provisioning-v3` and never merged
> to `main` were rewritten as individual records on 2026-08-09, before that branch landed,
> so those decisions ship in the current format rather than frozen in this one. The entries
> below now end at **2026-07-29**. Why only those 27, and why not the other 91:
> `2026-08-09-unmerged-decisions-ship-in-the-current-format.md`.
>
> Note when following an older `see DECISIONS.md <date>` citation from code or docs: 19
> dates below carry more than one entry (`2026-05-16` has eight), so a bare date may be
> ambiguous. Read the neighboring entries rather than assuming the first match. A citation
> to a date after 2026-07-29 belongs to one of the extracted records -- list
> `docs/decisions/2026-0[78]-*.md` and match on the subject.
