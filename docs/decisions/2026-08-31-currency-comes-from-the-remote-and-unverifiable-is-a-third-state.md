...
**That safety is an ordering argument, and it has one gap, accepted knowingly.** `provide-mast` (2200) always precedes `verify-mast` (2201) for the same module, and it is `provide-mast`'s fetch that establishes currency; verify's check inside an execute run is redundant. The redundancy is what makes "could not perform the redundant check" a safe pass. **If the route drops between 2200 and 2201 in a single run**, `provide-mast` passes (the checkout genuinely is at origin's tip), `verify-mast` exits 2, and `execute` records `verify: pass` with `fully_provisioned = true` -- and **nothing records that the currency check did not complete**, because `execute` does not write `validation.json` (`run-verify-only.ps1` is its only writer; see `server/prov/drift.py`).

The substantive claim stays true on that path -- the unit *is* current, established by the fetch seconds earlier -- so this is not the defect `#177` was. That was verify making a positive claim (`current`) it had not established; this is verify declining to re-record a fact another step already established. The first is a lie, the second a silence about something already known.

It is still an unrecorded "a check did not run", which is this epic's own smell, and it sits in the transient-drop family `#45` lives in. **Left as is for now, by decision, and expected to be revisited.** The cheap closure if it is: have `execute` write the unverifiable module names into the facts sidecar via `Write-MastModuleFacts` -- that mechanism exists for exactly "what a module FOUND, as opposed to whether it passed", `fleet-drift-report.py` already renders the facts matrix, and it needs no change to the bool. Deliberately not done here: it is beyond what `#177` asked, and the claim the manifest makes on that path is not false.
...
- **An execute run cannot report `unverifiable` at all.** The state exists only on the `validation.json` surface, so the transient-drop case described above leaves no trace: `fully_provisioned = true` on a run whose currency check did not complete. Accepted for now (the fact was established by `provide-mast` moments earlier), flagged as revisitable, and the closure is named in *What*.
