...
It also carries an operator control to **restart the unit**. That is the action most often wanted at the telescope, it is done by hand today, and routing it through the monitor keeps one component owning the unit's lifetime whether a restart was commanded or caused by a crash.
...
- Whether an operator restart overrides the in-progress veto. Refusing during `Exposing` is right for an automatic decision and wrong for a deliberate one, so the control probably reports the veto and offers to override rather than silently obeying it.
