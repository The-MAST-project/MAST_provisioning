...
- **Wait for the resources, do not merely check them.** The unit is not started until the share answers and the ImDisk volume on `D:` is mounted and usable. If either is missing the monitor waits, saying what it is waiting for, rather than starting a unit that would then write a night's products to the local disk.

  Waiting is the only mechanism available for `D:`, not merely the tidier one. The RAM disk is created by a **scheduled task**, `ImDisk Persistent Drive` -- a boot trigger running `imdisk -a -m D: -t vm -f "C:\MAST\Shared\MAST-15GB-indexes-5202+5203.img"` as `mast` with a stored password -- and a scheduled task cannot be named in an nssm `DependOnService`, so SCM has no way to order anything after it. The image is 15 GB and is loaded into RAM at mount, so "the task started" and "`D:` is usable" are minutes apart; a one-shot check would lose that race routinely rather than rarely. `Z:` is the monitor's business for a different reason: only session 1 can see it at all.
...
`mast-monitor` is a GUI application with a console area, and it narrates itself there: what it is waiting for, what it started or adopted, what it declined to restart and why. That console is what makes the waiting above acceptable rather than mysterious -- "waiting for D: (ImDisk image still loading)" on screen is a diagnosis, whereas a unit that silently never starts is a call-out. The same lines go to the daily log, because a console is lost the moment the monitor restarts and the interesting events are usually the ones that happened before anybody looked.

It also carries an operator control, **Forced restart**, which does three things in order: sends `abort` to the unit, waits for `/status` to report it idle, and only then restarts the Python process. Named for what it does to the observing rather than to the process -- it will interrupt work in progress, and an operator reaching for it should know that before clicking rather than after.

Aborting first is what resolves the in-progress veto rather than arguing with it. The monitor refuses to restart a unit that is `Exposing`, `Acquiring` or `Guiding`; a control that made the unit idle first is not fighting that rule, it is satisfying it. The wait needs a bound, because a unit wedged mid-exposure may never report idle -- and at that point the choice is between waiting indefinitely and killing it regardless, which is a decision the operator should be shown rather than one the monitor makes quietly.

Routing this through the monitor also keeps one component owning the unit's lifetime, whether a restart was commanded or caused by a crash.

PHD2 moves out of the unit process: `PHD2Connector.__init__` builds a `WatchedProcess` and starts `phd2.exe` itself, so PHD2's lifetime is tied to the unit's, which contradicts keeping the GUI applications up while the unit is stopped. It becomes a client of a PHD2 the monitor owns.

Hardware startup is decoupled from process startup. That separation is **already specified as MAST_unit#118** -- import binds names and does nothing else, construction creates objects without touching a device, acquisition powers and moves things on command -- with #114 as its one-off instance in `app.py`. This decision depends on #118 rather than restating it; what it adds is the operational reason. Restarting a process must not move hardware, which is the same requirement from a different direction, and the second stage is where the remaining exposure lies: `Mount.__init__` calls `power_on()` on an outlet and dispatches the ASCOM driver, so merely constructing a `Unit` acts. #118's own payoff is the test suite -- 25 of 53 unit test cases never execute off Windows -- and hardware-free provisioning verification on a VM falls out of the same change.

PHD2 moving out of the unit is likewise not new: **MAST_unit#111** already records that `PHD2Connector` does its startup in `__init__`, leaving `startup()` with nothing to do. This design needs it for a different reason -- PHD2 must outlive a unit stopped for maintenance -- but it is the same fix.
...
- Whether the wait for resources is bounded. Unbounded is defensible while the console and the log both say what is being waited for, but a unit that has been waiting since dusk for a share nobody is going to fix should be saying so somewhere louder than a console line.
- How long **Forced restart** waits for the unit to go idle before offering to kill it anyway. A unit wedged mid-exposure may never report idle, so there is a bound, and what happens at the bound is an operator's decision rather than the monitor's.
- The monitor is a GUI application, which makes the component that must not fail the one with a UI event loop. Whether supervision runs independently of the window -- so a wedged UI does not stop children being watched -- is a design constraint rather than a detail.
- The ImDisk task runs as `mast` with `LogonType=Password`, so the fleet holds a stored credential there as well as for auto-logon. Not introduced by this design, but it belongs in the same security picture.

  It cannot be moved into the vault, and not only for the obvious reason. Windows consumes that password itself, to create the logon session *before* the task's action runs, so no code of ours exists at the moment it is needed -- a file it could read would be read too late. The network objection is real too and independent: at boot trigger there is no `Z:` (nobody has logged on) and reaching the vault by UNC would need credentials to fetch the credentials.

  The stored password can still go, without the vault. The action is entirely local -- a 15 GB image on `C:` and no UNC anywhere in its arguments -- so the task can run as **SYSTEM** (`LogonType=ServiceAccount`, no password stored, inherently elevated), or stay as `mast` under **`LogonType=S4U`** ("do not store password"), whose only limitation, no network credentials, does not apply here. SYSTEM is simpler; S4U keeps the identity if anything depends on the mount being made by `mast`. Either wants confirming with a reboot that the drive letter is still visible from the interactive session before it is adopted.
