...
- `server/prov/driver.py` (the per-unit loop and the cadence loop), `transport.py`
  ...
  pure-logic modules `retention.py`, `staging_size.py`, `winrm_flap.py`
...
| 3 | GitHub token (`mast_github.txt`)             | RETIRED 2026-08-02 -- repos are public, clones are anonymous; the token is gone                             | n/a                                                                                                                          | closed                    | n/a                                                 |
...
- **Retired 2026-08-02:** there is no GitHub token. Every The-MAST-project repo is
  public, so `provide-mast.ps1` clones anonymously and `-AllowMissingGithubToken`
  is gone with the credential it guarded. Reverses only if a repo goes private,
  and then via an org-scoped `url.<...>.insteadOf` rewrite -- never a token baked
  into a remote URL (issue #17).
