...
- Do **not** treat `*>$null` (or `2>$null`) as a way to silence a native command. It hides the *text*; PowerShell still raises an error record for anything the command wrote to stderr. Under the default `$ErrorActionPreference = 'Continue'` that record lands in the transcript, so a **passing** run's operator-facing log carries `NativeCommandError` blocks that read like faults. Under `'Stop'` -- which every `verify-*.ps1` sets -- the record is **terminating**, and the script dies at a line whose exit code was the whole point of calling it. Both were measured on the dev VM for `pip show <absent package>`, which writes `WARNING: Package(s) not found` and exits 1: a normal answer, fatal to a script that asked the question. When a probe's *exit code* is the answer you want, wrap the call so the preference is forced for its duration and return `$LASTEXITCODE` -- `Get-ProbeExitCode` in `server/providers/python/provide-python.ps1` and `Invoke-Probe` in its `verify-python.ps1` are the shape. See `docs/decisions/2026-08-31-virtualenv-is-dropped-rather-than-its-failure-reported.md`.
