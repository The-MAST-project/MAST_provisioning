...
## [2026-08-03] Z: belongs to the operational share, and must be mapped in the SYSTEM session
...
`mast-unit`, `mast-phd2`, `mast-pwi4` and `mast-pwshutter` all run as **LocalSystem**
(confirmed on mast03). Windows drive letters are per-logon-session. Execute runs as the
autologon `mast` user, so even a *correct* mapping it made would be invisible to the
process that calls `Filer` -- and `Filer.__init__` silently substitutes `C:\MAST` when
the `Z:` probe fails. That silent substitution is what made the 2026-07-14 mast03
tracking-test exposures look lost: they were on `C:` all along.
...
- Mounting is **best-effort**, installing the mechanism is not: a unit provisioned in
  the lab cannot reach its site controller, so the provider warns (`shared_mount_warn`)
  and the run continues. Verify fails only on install-level problems, and always
  records the last mount outcome -- a silently unmounted `Z:` is the failure mode being
  designed out.
- Existing units converge on the next run: the provider deletes the per-user mapping
  left behind by earlier provisioning before establishing the SYSTEM one.
- The VM harness does not plant the share credential, so `mast-shared-mount` will always
  report a warn there. That is correct -- there is no controller to reach.
- The deeper fix belongs upstream: `Filer` should take a UNC path from
  `controller_host` instead of a drive letter, and should log loudly rather than
  silently fall back to `C:`. A drive letter is a per-session concept being used to
...
## [2026-08-03] The nomachine provider pins server.cfg and verifies the serving state, not the service state
...
3. **Verification asserted the wrong thing.** `verify-nomachine.ps1` passed if
   ...
   is the "post-deploy smoke test" that the [2026-05-24] entry deferred
...
- The fleet was recovered by hand on 2026-08-03 (both keys set, `nxservice`
  restarted); all four units serve on 4000 again. The provider changes make that
  recovery reproducible rather than a remembered incantation.
- `SessionHistory 0` means **no NoMachine session history** on the units: who
  connected and when is no longer recorded by NoMachine. Accepted -- the units
  are reached through a single shared `mast` account, so the history had little
  audit value, and `SessionHistory -1` (retain forever, never trim) was the
  alternative if that changes.
- The update check being off means new NoMachine releases must be noticed
  manually. That is deliberate: the version nag was unwanted, and the module
  pins `9.0.188` as a staged asset anyway.
- The `libnxdb.dll` null deref and the persistently failing rename should go
  upstream to NoMachine. The evidence is preserved on the units in
  `var\log\trace.log` (stack + fault address per crash), `server.log` (the
  rename failure immediately preceding each crash), and `service.log` (the
  give-up). **Why the rename fails 100% of the time is still unexplained** --
  deterministic rather than racy, so it points at a persistent open handle on
  `history` rather than a Defender scan race.
- Verify now fails loudly on a unit in this state instead of reporting healthy,
  so a recurrence surfaces at the next provisioning/verify pass rather than the
  next time someone tries to connect.

---

## [2026-08-03] Stage-6 migration is a tool with preconditions, and renames rather than deletes
...
**Verified against the VM, as the real procedure rather than a manual `rm`:** dry
run reported `ready`, `--apply` stopped the service, renamed the tree to
`repos.retired-20260803-091325`, restored Running, and post-checks passed. A
follow-up provisioning cycle then produced the first fully clean module verify --
`mast verify ok: 3 clone(s) current under C:\MAST\src` -- with the only
remaining harness failure being the pre-existing `unit heartbeat` on `:8000`,
which fails identically on the pre-migration baseline.

**Implications:** the non-standard-entry warning considers **directories** only.
Listing every entry buried the signal under ~24 provisioning sidecar logs
(`<repo>.git-trace.log` and friends), which defeats the purpose -- on mast02 the
entries that matter (`mast-claude-config`, `PlaneWave_PlateSolve3_Catalog`) would
have been invisible in that wall of names. Note also that a rename **retains**
those sidecar logs, which per issue #17 contain the (now expired) token in
`GIT_TRACE` output; `--purge` is the tidier end state once a unit is trusted.
...
## [2026-07-20] Bootstrap file is `C:\WIS\config.toml` with an injected `machine_role`; no `MAST_PROJECT`
...
**Implications:** breaking bootstrap change — a unit on the new `common` with an old
`C:\WIS\<role>.toml` (or a `config.toml` lacking `machine_role`) fails fast at startup, by
design; units are fixed on the next provision. Landed on `eli/machine-role`, branched off and
targeting `eli/provisioning-v3` (not `main`). Not yet exercised on a VM/unit — the
provisioning verification is deferred (epic #15). Supersedes the 2026-06-29 `config-bootstrap`
entry's `C:\WIS\unit.toml` + `MAST_PROJECT` design.
