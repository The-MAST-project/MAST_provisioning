...
Gathers each unit's C:\\MAST\\installed-manifest.json (written by
execute-mast-provisioning.ps1) over SSH and prints a cross-unit comparison:
a per-unit summary plus a module-version matrix that flags where units diverge.
Read-only -- it never changes anything on a unit.

Why SSH (not WinRM): SSH reaches units from any egress, whereas the units' WinRM
listener is LocalSubnet-scoped, so a cross-subnet host (e.g. labcomp) cannot WinRM
to them. A --winrm mode can be added later for a same-subnet prov server.

This is the MVP of the "Version / Drift Detection" feature in
autonomous-provisioning-requirements.md. It trusts the static installed-manifest.json
(explicitly acceptable as an audit artifact today) and treats a missing manifest as a
first-class drift signal. Growth path: computed/live manifests, tiered self-validation,
Prometheus surfacing -- all behind this same report shape.
...
Exit codes: 0 = every unit IN SYNC; 2 = drift / missing / unreachable found; 1 = tool error.
"""
...
def gather_unit(host: str, cred: dict[str, str], connect_timeout_s: int) -> UnitRecord:
    """Read installed-manifest.json from one unit over SSH (read-only)."""
    ...
    try:
        script = (
            ...
            f"{{ Get-Content -LiteralPath '{MANIFEST_PATH}' -Raw }} "
            f"else {{ '{NO_MANIFEST_SENTINEL}' }}"
        )
        resp = session.run_ps(script)
        out = resp.std_out.decode("utf-8-sig", errors="replace").strip()
        if not out or NO_MANIFEST_SENTINEL in out:
            return UnitRecord(host=host, status="no-manifest")
        try:
            obj = json.loads(out)
        except json.JSONDecodeError as exc:
            return UnitRecord(host=host, status="parse-error", error=str(exc))
        return _manifest_from_obj(host, obj)
...
def render(units: list[UnitRecord], reference: UnitRecord | None, cmp: dict) -> str:
    lines: list[str] = []
    cols = ([reference] if reference else []) + units
    host_w = max([len(u.host) for u in cols] + [9])

    lines.append("=== Fleet summary ===")
    hdr = f"{'unit'.ljust(host_w)}  {'status'.ljust(11)}  {'payload'.ljust(12)}  {'git'.ljust(12)}  installed_at"
    lines.append(hdr)
    lines.append("-" * len(hdr))
    for u in cols:
        verdict = "REFERENCE" if u is reference else cmp["verdicts"].get(u.host, "?")
        detail = f"  {u.error}" if u.error else ""
        lines.append(
            f"{u.host.ljust(host_w)}  {verdict.ljust(11)}  {_short(u.payload_hash).ljust(12)}  "
            f"{_short(u.git_sha).ljust(12)}  {u.installed_at or '-'}{detail}"
        )
    ...
    problems = [u.host for u in units if cmp["verdicts"].get(u.host) != "IN SYNC"]
    if problems:
        lines.append(f"RESULT: DRIFT/gaps on {len(problems)} unit(s): {', '.join(problems)}")
    else:
        lines.append("RESULT: all units IN SYNC")
    return "\n".join(lines)


def write_csv(path: Path, units: list[UnitRecord], cmp: dict) -> None:
    modules = cmp["modules"]
    with path.open("w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["host", "status", "verdict", "payload_hash", "git_sha", "installed_at"] + modules)
        for u in units:
            w.writerow(
                [u.host, u.status, cmp["verdicts"].get(u.host, "?"), u.payload_hash or "", u.git_sha or "", u.installed_at or ""]
...
def main() -> int:
    ...
    if args.from_json:
        ...
        try:
            ...
            return 1
        print(f"Gathering installed-manifest.json from {len(hosts)} unit(s) over SSH...", file=sys.stderr)
        units = []
        for h in hosts:
            rec = gather_unit(h, cred, args.connect_timeout)
            print(f"  {h}: {rec.status}", file=sys.stderr)
            units.append(rec)
    ...
    if args.csv_out:
        write_csv(Path(args.csv_out), units, cmp)

    print(render(units, reference, cmp))

    if not units:
        return 1
    return 0 if all(cmp["verdicts"].get(u.host) == "IN SYNC" for u in units) else 2
