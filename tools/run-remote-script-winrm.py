...
def ps_escape_single(value: str) -> str:
    """Escape for embedding inside PowerShell single-quoted strings."""
...
def build_remote_invoke(
    ...
    *,
    ...
    """
    rid = ps_escape_single(run_id)
    repo = ps_escape_single(script_repo_rel.replace("\\", "/"))
...
def load_vault(path: Path) -> tuple[list[str], str]:
    data = json.loads(path.read_text(encoding="utf-8"))
    raw_user = data["unit"]["user"]
    pw = data["unit"]["pass"]
    candidates: list[str] = [raw_user]
    if raw_user.startswith(".\\"):
        candidates.append(raw_user[2:])
    elif "\\" in raw_user:
        candidates.append(raw_user.split("\\")[-1])
    else:
        candidates.append(f".\\{raw_user}")
    seen: set[str] = set()
    ordered: list[str] = []
    for c in candidates:
        if c not in seen:
...
def open_session(host: str, users: list[str], password: str, *, quiet: bool) -> winrm.Session:
    extra: list[str] = []
    if re.match(r"^\d{1,3}(\.\d{1,3}){3}$", host):
        for u in users:
            if "\\" not in u and not u.startswith("."):
                extra.append(f"{host}\\{u}")
            elif u.startswith(".\\"):
                extra.append(f"{host}\\{u[2:]}")
    try_order: list[str] = []
    seen_u: set[str] = set()
...
def wait_for_winrm_ready(
    ...
    *,
    ...
    if timeout_s <= 0:
        return open_session(host, users, password, quiet=quiet)
...
def main() -> int:
    ...
    users, password = load_vault(vault_path)
