...
def main() -> int:
    ...
    ap.add_argument("--smoke-file", required=True, help="Path of smoke marker to write")
    ap.add_argument(
        "--allow-missing-avx",
        action="store_true",
    ...
    if not ctrl_ok:
        print(
            f"FAIL: could not resolve the controller from the unit's config: {ctrl_detail}. "
    ...
    if not fits_path.exists():
        print(
            f"FAIL: full-frame FITS not present at {fits_path}. "
    ...
    if not ok:
        print(
            f"FAIL: astrometry indexes not available ({reason}). "
    ...
    if result is None or not getattr(result, "succeeded", False):
        ...
        if corrupt:
            ...
            for p in corrupt:
                print(f"  {p}")
            print(
                "These files are corrupt (e.g. missing kdtree header) and must be rebuilt or "
        ...
        if args.allow_missing_avx and ("killed by signal 4" in err_text or "SIGILL" in err_text):
    ...
    body = f"mastrometry_ok solve=ok controller={controller} ra_hours={ra_hours} dec_degs={dec_degs} matched={matched}"
    _write_smoke(smoke_path, body)
    print(body)
    return 0
