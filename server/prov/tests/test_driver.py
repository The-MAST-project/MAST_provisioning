...
def test_resolve_modules_precedence(root):
    ...
    assert drv._resolve_modules(R.UnitEntry(hostname="m", site="wis")) == ["ascom", "git", "mast"]
    # A registry entry says what this unit's full set IS, so it wins over discovery.
    assert drv._resolve_modules(R.UnitEntry(hostname="m", site="wis", modules=["git"])) == ["git"]
...
def test_resolve_modules_ignores_the_cli_subset(root):
    ...
    assert drv._resolve_modules(R.UnitEntry(hostname="m", site="wis", modules=["ascom"])) == ["ascom"]
