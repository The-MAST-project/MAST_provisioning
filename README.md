...
A verify step has **three** outcomes, not two. Exit `0` is a pass and exit `1` a failure; **exit `2` means every check that could run passed and at least one could not** — typically the unit cannot reach `origin`, so a clone's revision cannot be compared against it. That is recorded per module in `C:\MAST\status\validation.json` as `unverifiable` and does **not** count toward this script's exit code: an operator's local run should not go red for a network condition. `fleet-drift-report.py` reports it apart from both pass and fail, and it suppresses the all-clear `RESULT` line — an unanswered currency question is not a clean bill. See `docs/decisions/2026-08-31-currency-comes-from-the-remote-and-unverifiable-is-a-third-state.md`.
